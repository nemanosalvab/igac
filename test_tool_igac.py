...
class Toolbox(object):
  def __init__(self):
    """Define the toolbox (the name of the toolbox is the name of the .atbx )"""
    self.label = "Toolbox"
    self.alis = "toolbox"
...
class Tool(object): 
  ...
  def getParameterInfo(self):
    ...
    params = [
      arcpy.Parameter(displayName="Input feature class",
  ...
  def execute(self, parameters, messages):
      ...
      coincidence_percent = float(parameters[1].value)
      buffer_radius = float(parameters[2].value)  # Assuming the parameter is a numeric input
      output_shapefile = parameters[3].valueAsText
      print(output_shapefile)
      self.compare_fields_within_buffer(shapefile_path, field_to_compare1, field_to_compare2, coincidence_percent,buffer_radius, output_shapefile)
  ...
  def compare_fields_within_buffer(self, shapefile, field1, field2, coincidence_percent,buffer_radius, output_shapefile):
      ...
      arcpy.management.CopyFeatures(selected_features, output_shapefile+"\\result\\coincidencia.shp")
...
if __name__ == '__main__':
    ...
    shapefile_path = os.path.join(dirname , 'shapes\\BNNG_Boyaca\\BNNG_Boyaca.shp').replace("\/","\\")
    # Spatial reference set to GCS_WGS_1984
    spatial_reference = arcpy.SpatialReference(9377)
    #field_to_compare1 = arcpy.GetParameterAsText(1)
    field_to_compare1 = "NOMBRE_GEO"
    #field_to_compare2 = arcpy.GetParameterAsText(2)
    field_to_compare2 = "ngnoficial"

    #coincidence_percent = float(arcpy.GetParameterAsText(3))
    coincidence_percent = 80
    
    #selected_point = (float(arcpy.GetParameter(4)), float(arcpy.GetParameter(5)))  # Assuming the parameters are numeric
    #buffer_radius = float(arcpy.GetParameter(6))  # Assuming the parameter is a numeric input
    buffer_radius = 1000  # Assuming the parameter is a numeric input
    
    #output_shapefile = arcpy.GetParameterAsText(7)
    output_shapefile = os.path.join(dirname , 'shapes\\output').replace("\/","\\")

    #Tool.execute()

    # Call the function to compare fields within the buffer and create the output shapefile
    call = Tool()
    call.compare_fields_within_buffer(shapefile_path, field_to_compare1, field_to_compare2, coincidence_percent,buffer_radius, output_shapefile)
